...
export default async function NewsPage({ params: { id } }: Props) {
  ...
  return (
    <>
      <div className={styles.header} style={{ paddingTop: "150px" }}>
        ...
        <h4>{news.data.attributes.title}</h4>
        <div className={styles.publishedDate}>
          Published on &nbsp;
          {FormatNewsPublishedDate(
            news.data.attributes.customPublishedDate ||
              news.data.attributes.publishedAt
          )}
        </div>
      </div>
      <div
