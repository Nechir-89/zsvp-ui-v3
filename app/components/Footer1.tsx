  ...
  return (
    <div className={`container ${styles.contentWraper}`}>
      <div className={styles.col1}>
        {
          aboutItems?.data?.map((aboutItem: AboutItem) => <span key={aboutItem.id} className='footer-nav-item'>
            <Link href={`/about#${removeSpace(aboutItem.attributes.title)}`}>{aboutItem.attributes.title}</Link>
          </span>)
        }
          ...
          contacts.data.map(contact =>
            <div key={contact.id} className={styles.contactWraper}>
              <Image
                ...
                height={24}
              />
              <span className={`footer-nav-item ${styles.footerNavItem}`}>{contact.attributes.value}</span>
            </div>
          )
        }
          ...
          <div className={styles['social-icons']}>
            {
              medias?.data?.map(media =>
                <Link href={media.attributes.link}
                  target='_blank'
                  key={media.id}
                  rel='noopener noreferrer'
                >
          ...
          <div className={styles['map-icon']}>
            ...
            >
              <Image
                ...
                alt={location.data.attributes.icon.data.attributes.alternativeText}
                width={24}
                height={24}
              />
            </Link>
